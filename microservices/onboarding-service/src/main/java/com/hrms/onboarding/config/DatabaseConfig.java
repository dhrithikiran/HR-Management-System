package com.hrms.onboarding.config;
...
public class DatabaseConfig {
    ...
    static {
        ...
        if (DB_PASS == null || DB_PASS.trim().isEmpty()) {
            logger.error("Onboarding Service: DB_PASSWORD environment variable is missing!");
            throw new IllegalStateException("Database password is required. Please set the DB_PASSWORD environment variable.");
        }

        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            logger.info("Onboarding Service: MySQL JDBC Driver loaded.");
        } catch (ClassNotFoundException e) {
            logger.error("Onboarding Service: MySQL Driver class not found", e);
